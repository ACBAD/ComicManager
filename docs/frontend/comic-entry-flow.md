...
批量录入是单独的按钮流程，位于“未完成漫画”列表。处理当前筛选内所有未入库漫画，
并不在单部录入页面自动执行。每部先查询全部精确映射：全部已有映射时直接提交；
只有 group 类未映射时，按来源原名创建或复用同组通用标签、保存映射，再提交。
存在其他未映射类型时整部跳过，不先写入部分 group。批量操作不经过单部最终复核页，
仍使用相同的 commit 接口检查最新来源的标签映射，且不自动覆盖已有记录。

批量停止会等待当前漫画结束；运行时不能切换页面。逐部结果保留录入、跳过和失败的原因，
结束后刷新 CM 对照，未满足条件的漫画仍可进入原来的手动工作台。
