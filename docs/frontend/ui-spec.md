...
本次 commit 返回的 Comic。从“未完成漫画”进入且 CM 已有记录时，复核页明确显示
“确认更新漫画”和替换范围，确认后才执行更新。

## 未完成漫画列表

独立入口为 `#/pending`。顶部提供重新扫描、扫描进度和停止操作，扫描完成后显示
未完成/已完成计数及两边记录数。搜索、未完成原因与 DMB 状态筛选位于列表上方。
每行展示漫画标题、两边更新时间、未完成原因和整理入口；每页 20 条，支持输入页码后
点击“跳转”或按回车，页码限制在当前筛选结果的有效范围内。
窄屏将时间与操作排在标题下方，导航单独占一行。
