...
- 打开队列默认先读取全部 CM，再请求 DMB `mode=all, orderby=updated_at, order=DESC`。
  逐部对比，到首次遇到 CM 已存在且 `CM.updated_at > DMB.updated_at` 的记录为止。
...
- 队列摘要按 DMB 地址保存在 `localStorage["comicmanager.entryQueue:" + dmbUrl]`。
  刷新后重新读取 CM 并做 anchor 扫描；存储不可用时仍支持本页面内的队列。
...
- 每次提交后，通过现有标题精确查询接口按 ID 找到持久化 Comic，并重新读取 DMB，
